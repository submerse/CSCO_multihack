...
def get_head_bone_index(pm, ent_ptr):
    if ent_ptr in head_bone_cache:
        return head_bone_cache[ent_ptr]

    base_z = pm.read_float(ent_ptr + POS_Z)  # entity base Z (feet)
    ...
    for bone_idx in candidate_bones:
        bone_offset = bone_idx * 0x30
        bx = pm.read_float(bonematrix_ptr + bone_offset + 0x0C)
        by = pm.read_float(bonematrix_ptr + bone_offset + 0x1C)
        bz = pm.read_float(bonematrix_ptr + bone_offset + 0x2C)
    ...
    if best_bone is not None:
        head_bone_cache[ent_ptr] = best_bone
...
def aimbot_thread(pm, entity_list, pitch_addr, yaw_addr):
    ...
    movement_threshold = 0.01  # minimum movement (in world units) to consider enemy moving
        ...
        try:
            ...
            for i in range(1, MAX_ENTITIES):
                try:
                    ...
                    # Determine horizontal movement direction for lead
                    ...
                    if prev_pos is not None:
                        ...
                        # Apply movement threshold to avoid jitter
                    ...
                    angle = calc_angle(local_pos, enemy_pos)

                    # Apply horizontal lead only if enemy is moving left or right
...
def gui_thread(pm, entity_list):
    ...
    smooth_slider = tk.Scale(root, from_=0.0, to=80.0, resolution=0.01, orient="horizontal")
