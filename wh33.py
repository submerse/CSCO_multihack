...
def esp_loop(pm, client, entity_list, canvas, root):
    ...
    def update():
        if not shared["running"] or not canvas.winfo_exists():
            return  # stop loop if canvas closed or stopped
        ...
        try:
            ...
            for i in range(MAX_ENTITIES):
                try:
                    ...
                    if not ent_ptr or i == shared.get("local_index", -1):
                        continue
            ...
            # no manual canvas.update() needed — Tkinter mainloop handles it

        except Exception:
            pass

        root.after(10, update)  # schedule next frame ~60 FPS

    root.after(0, update)  # start loop immediately
