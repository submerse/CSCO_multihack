...
# Bone skeleton model — indices to read + connections to draw (full-bones ESP)
BONE_STRIDE      = 0x30  # bytes per bone in the bone matrix
BONE_INDICES     = [0, 1, 2, 3, 4, 6, 7, 8, 11, 12, 14, 15, 16, 42, 67, 68, 69, 74, 75]
BONE_CONNECTIONS = [
    (0, 1), (1, 2), (2, 3), (3, 4),     # spine
    (4, 11), (11, 12),                  # head/neck
    (4, 14), (14, 15), (15, 16),        # left arm
    (4, 6), (6, 7), (7, 8),             # right arm
    (0, 67), (67, 68), (68, 69),        # left leg
    (0, 42), (42, 74), (74, 75),        # right leg
]
...
shared = {
    ...
    # ESP
    "team_mode":  "auto",
    "bone_mode":  "head",   # "all" = full skeleton, "head" = head circle, "off" = none
    # aimbot
    "recoil":               20,
    "entity_info":          [],
    "aim_active":           False,
    ...
    "tb_datatype":    "FLOAT",
    "tb_delay":       0,    # ms delay between hit detection and fire (0-250)
...
def get_bone_world_pos(pm, bonematrix_ptr, bone_id):
    base = bonematrix_ptr + bone_id * BONE_STRIDE
    return (pm.read_float(base + 0x0C),
            pm.read_float(base + 0x1C),
            pm.read_float(base + 0x2C))
    ...
    def render(cmds):
        ...
        for cmd in cmds:
            ...
            if kind == "box":
                ...
                canvas.create_line(cmd[1], cmd[2], cmd[3], cmd[4],
                                   fill=NORD["green"], width=2)
            elif kind == "fov":
                r = cmd[3]
                canvas.create_oval(cmd[1]-r, cmd[2]-r, cmd[1]+r, cmd[2]+r,
        ...
        try:
            ...
            for i in range(MAX_ENTITIES):
                try:
                    ...
                    if pm.read_bytes(ent_ptr + 0xE9, 1)[0]:
                    ...
                    bone_mode = shared["bone_mode"]
                    if bone_mode != "off":
                        try:
                            bm_ptr = pm.read_uint(ent_ptr + 0x26A0)
                            if bm_ptr and bone_mode == "head":
                                best_bone  = None
                                highest_bz = -float("inf")
                                for bone_id in [6, 8, 12]:
                                    off  = bone_id * BONE_STRIDE
                            ...
                            elif bm_ptr and bone_mode == "all":
                                bones = {bid: get_bone_world_pos(pm, bm_ptr, bid)
                                         for bid in BONE_INDICES}
                                for start_bone, end_bone in BONE_CONNECTIONS:
                                    sp_ = bones.get(start_bone)
                                    ep_ = bones.get(end_bone)
                                    if sp_ and ep_:
                                        ss = world_to_screen(sp_, view_matrix, sw, sh)
                                        es = world_to_screen(ep_, view_matrix, sw, sh)
                                        if ss and es:
                                            cmds.append(("line", ss[0], ss[1], es[0], es[1]))
                        except:
                            pass
            ...
            if not shared["tb_enabled"]:
                ...
                if prev_value is None:
                    ...
                    with mouse.Controller() as m:
                        time.sleep(shared["tb_delay"] / 1000.0)
...
def build_config_window(root):
    ...
    _section(tab_tb, "FIRE DELAY (MS)").pack(fill="x", **PX, pady=(12, 0))
    tb_delay_slider = _slider(tab_tb, 0, 250, 1,
                              cmd=lambda v: shared.update({"tb_delay": int(float(v))}))
    tb_delay_slider.set(shared["tb_delay"])
    tb_delay_slider.pack(**PX)
    ...
    _section(tab_esp, "BONE ESP").pack(fill="x", **PX, pady=(14, 0))
    bone_labels = {"All Bones": "all", "Head Only": "head", "Off": "off"}
    bone_inv    = {v: k for k, v in bone_labels.items()}
    bone_var    = tk.StringVar(value=bone_inv.get(shared["bone_mode"], "Head Only"))
    bone_combo  = ttk.Combobox(tab_esp, values=list(bone_labels.keys()),
                               textvariable=bone_var, state="readonly",
                               font=("Segoe UI", 9), width=30)
    bone_combo.pack(**PX, pady=(4, 0), anchor="w")
    bone_combo.bind("<<ComboboxSelected>>",
                    lambda e: shared.update({"bone_mode": bone_labels[bone_var.get()]}))
